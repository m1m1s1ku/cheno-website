...
export class ContactController extends Page {
      ...
      <form
        ...
        @input=${(e: KeyboardEvent) => {
          ...
          const fields = Array.from(
            this.querySelectorAll(".field mwc-textfield, .field mwc-textarea")
          ) as TextField[];
          this.send.disabled = fields.some(
            (field) => field.checkValidity() === false
          );
        }}
      >
        <div class="field">
  ...
  public render(): void | TemplateResult {
    return html`
      <div id="contact" class="contact" role="main">
        <div class="side">
          <h2>Contact</h2>
          <h4 id="helper" class="helper">Réponse rapide, c'est promis !</h4>
          ${this._contactForm}
        </div>
        <div class="side layer">
          <h2>Besoin d'une présentation ?</h2>
          <p>
            Des visites dans le "Jardin des Sculptures" sont possibles sur
            simple demande, n'hésitez pas !
          </p>
            ...
            : html`${this.max !== 0
                ...
                : html`
                    <mwc-button
                      class="book"
                      raised
                      label="Générer le book"
                      icon="picture_as_pdf"
                      @click=${this._generate}
                    ></mwc-button>
                  `}`}
          <p class="ecology-smile">
            (à n'imprimer que si nécessaire ! <mwc-icon>tag_faces</mwc-icon>)
          </p>
          ${this._socialLinks}
        </div>
      </div>
  ...
  private async _pagesMaker(
    doc: PDFDocument,
    fontkit: typeof import("@pdf-lib/fontkit")
  ) {
    const logoBytes = await fetch(
      location.origin + "/assets/images/logo.png"
    ).then((res) => res.arrayBuffer());

    const displayFontBytes = await fetch(
      location.origin + "/assets/fonts/andika.ttf"
    ).then((res) => res.arrayBuffer());
    const normalFontBytes = await fetch(
      location.origin + "/assets/fonts/comfortaa.ttf"
    ).then((res) => res.arrayBuffer());
    ...
    return {
      ...
      first: async (page: PDFPage) => {
        ...
        const subjectWidth = displayFont.widthOfTextAtSize(
          subject,
          subjectSize
        );
      ...
      series: async (description: ReadonlyArray<WPCategory>, maker) => {
        ...
        for (const category of description) {
          ...
          const categorySize = displayFont.widthOfTextAtSize(
            category.name,
            titleSize
          );
          ...
          for (const sculpture of category.sculptures.nodes) {
            const extension = sculpture.featuredImage.node.sourceUrl.substr(
              sculpture.featuredImage.node.sourceUrl.lastIndexOf(".") + 1
            );
            ...
            let sculptureDimension = sculptureImage.scale(0.4);
            if (prevY > 100) {
            ...
            if (isSub) {
              const detailCatSize = displayFont.widthOfTextAtSize(
                category.name,
                detailTitleSize
              );
            ...
            try {
              if (sculptureImage) {
                ...
                if (prevY - padding - lines.length * 10 < 0) {
                  ...
                  sculptureDimension = sculptureImage.scale(0.2);
  ...
  private async _generate() {
    ...
    const fontkit = await import(
      /* webpackChunkName: "pdf-lib-fontkit" */ "@pdf-lib/fontkit"
    );
    const doc = await PDFDocument.create();
    const maker = await this._pagesMaker(doc, fontkit.default);
    maker.meta();
    ...
    const categories = requestR.data.categories.nodes.filter(
